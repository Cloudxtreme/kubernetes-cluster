...
function create_storage {
	...
	lvcreate -L ${STORAGE_SIZE} -n ${PARTITION_PREFIX}kubernetes-\${name}-storage ${LVM_VG}

	echo \"format data volum ...\"
	wipefs /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-\${name}-storage
	mkfs.xfs -i size=512 /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-\${name}-storage
}
