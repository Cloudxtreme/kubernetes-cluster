...
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-master,bus=virtio,cache=${DISK_CACHE},io=${DISK_IO} \\
...
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-storage,bus=virtio,cache=${DISK_CACHE},io=${DISK_IO} \\
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-storage-data,bus=virtio,cache=${DISK_CACHE},io=${DISK_IO} \\
...
for ((i=0; i < ${WORKER_AMOUNT}; i++)) do
	...
	--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-worker\${i},bus=virtio,cache=${DISK_CACHE},io=${DISK_IO} \\
	--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-worker\${i}-storage,bus=virtio,cache=${DISK_CACHE},io=${DISK_IO} \\
