...
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-master,bus=virtio,cache=${NODE_DISK_CACHE},io=${NODE_DISK_IO} \\
...
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-storage,bus=virtio,cache=${NODE_DISK_CACHE},io=${NODE_DISK_IO} \\
--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-storage-data,bus=virtio,cache=${STORAGE_DISK_CACHE},io=${STORAGE_DISK_IO} \\
...
for ((i=0; i < ${WORKER_AMOUNT}; i++)) do
	...
	--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-worker\${i},bus=virtio,cache=${NODE_DISK_CACHE},io=${NODE_DISK_IO} \\
	--disk /dev/${LVM_VG}/${PARTITION_PREFIX}kubernetes-worker\${i}-storage,bus=virtio,cache=${STORAGE_DISK_CACHE},io=${STORAGE_DISK_IO} \\
