...
function generate_configs {
  ...
  # Avaiable Version: https://gcr.io/v2/google-containers/hyperkube-amd64/tags/list
	...
	NODE_DISK_CACHE=${NODE_DISK_CACHE:="none"}
	NODE_DISK_IO=${NODE_DISK_IO:="native"}

	DOCKER_DISK_CACHE=${DOCKER_DISK_CACHE:="none"}
	DOCKER_DISK_IO=${DOCKER_DISK_IO:="native"}

	STORAGE_SIZE=${STORAGE_SIZE:="10G"}
	SYSTEM_SIZE=${SYSTEM_SIZE:="10G"}
	DOCKER_SIZE=${DOCKER_SIZE:="10G"}

	DISK_PREFIX=${DISK_PREFIX:=""}
	VM_PREFIX=${VM_PREFIX:=""}

	MASTER_MEMORY=${MASTER_MEMORY:="1024"}
	STORAGE_MEMORY=${STORAGE_MEMORY:="1024"}
	WORKER_MEMORY=${WORKER_MEMORY:="3072"}
